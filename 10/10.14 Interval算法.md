...
仍然是排序贪心的逻辑，首先按照start升序排序，然后遍历intervals，当有两个interval相互重叠的时候，则删去end比较大的那个。具体代码如下所示：
```Java
...
public int minMeetingRooms(List<Interval> intervals) {
    ...
    for (Interval interval : intervals) {
        int start = map.getOrDefault(interval.start, 0);
        map.put(interval.start, ++start);
        int end = map.getOrDefault(interval.end, 0);
        map.put(interval.end, --end);
    }
